...
const listCampingHistory = async (req, res) => {
    ...
    try {
        const history = await CampingHistory.find({ user: userId })
            ...
            .populate('activity', 'name description') // Aktivite bilgilerini ekle
            .populate('accommodation', 'name location price') // Konaklama bilgilerini ekle
            .populate('restaurant', 'name location') // Restoran bilgilerini ekle
            .populate('favorites', 'name type'); // Favori bilgilerini ekle
